...
$fullQuery = "select * from $tableName ".$whereq." order by id";
	...
	for($i=1;$i< mysql_num_fields($result);$i++){
		if ($fieldEdit[$i]["type"]=="__IMAGE" || $fieldEdit[$i]["type"]="__FILE") {
			if($_FILES["file$i"]["name"]!=""){
				...
				copyimg($_FILES["file$i"]["tmp_name"],"../".$fieldEdit[$i]["location"].$tm.".".$ext);
...
if($_GET["action"]=="" || $_GET["action"]=="list" || $_GET["action"]=="del"){
	...
	echo "<td class=listHeader width=50><div align=right>Action</div></td>";
	echo "</tr>";
	...
	while ($rows = mysql_fetch_row($result) ) {
		...
		for($i=0;$i<mysql_num_fields($result);$i++ )
			...
			if($dontShow[$i]==true)continue;
				...
				}else if ($fieldShow[$i]=="__IMAGE"){
					if (file_exists($rows[$i])) {
						$fvalue = "<a href=\"".$rows[$i]."\"><img border=0 height=100 src=\"".$rows[$i]."\"/>"."</a>";
					}
					else $fvalue="&nbsp;";
				}
		...
		$id=$rows[0];
		echo "<td class=listTD><nobr><a href=$opageName?action=edit&id=$rows[0]".$where_xtra.">Edit</a> | <a href=\"javascript:confirmgo('$opageName?action=del&id=$rows[0]$where_xtra','Are you sure you want to delete?')\">Delete</a>$addedit_xtra</nobr></td></tr>";
	}
...
else if(@$_GET["action"]=="doadd")
{
	...
	echo "<td class=addHeader>&nbsp;</td><td class=addTD><input type=Submit name=submit value=Add onClick=\"return checkMe('input',".$required.");\">$hiddenstr
...
{	
	...
	echo "<td class=editHeader>&nbsp;</td><td class=editTD><input type=Submit Name=Submit Value=\"Commit Change\" onClick=\"return checkMe('edit',".$required.");\">$hiddenstr 
	<a name=ab href=$postaction>Back to list</a><input type=hidden name=id value=$rows[0]><input type=hidden name=action value=edit>
	</td></tr></table>";
	if($required)echo "<table><tr><td><p><font color=red size=1>[*] means required fields</font></p></td></tr></table>";
}
