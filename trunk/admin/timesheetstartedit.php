...
<table width="100%"  border="0" cellspacing="0" cellpadding="20">
      <tr>
		...
		if($_POST["starttime"]){
			$timeid = select_field("userworktime where starttime=".$_GET["time"]." and userid=".$_GET["empid"],"id");
			if($timeid){
				$newstarttime = mktime($_POST["starttime"][3],$_POST["starttime"][4],0,$_POST["starttime"][1],$_POST["starttime"][0],$_POST["starttime"][2]);
				$up = mysql_query("update userworktime set starttime=".$newstarttime." where starttime=".$_GET["time"]." and userid=".$_GET["empid"]);
				if($up==1){
