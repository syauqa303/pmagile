...
<table width="100%"  border="0" cellspacing="0" cellpadding="20">
      <tr>
		...
		$quserList = mysql_query("select distinct emp.id,emp.name from emp,userprojectworktime where emp.id=userprojectworktime.userid and userprojectworktime.projectid=".$pid." order by emp.name");
 ?>
		<table width="100%" border="0" cellpadding="3" cellspacing="0" border="1">
            <tr>
              <td width="50%" class="viewHeader">User</td>
              <td width="50%" class="viewHeader">Hours Elapsed </td>
            </tr>
            <? while ($userList = mysql_fetch_row($quserList)){ ?>
            <tr>
              <td width="50%" ><a href="userprojectworktime.php?uid=<?=$userList[0]?>&pid=<?=$pid?>"><?=$userList[1]?></a></td>
