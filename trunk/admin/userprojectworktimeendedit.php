...
<table width="100%"  border="0" cellspacing="0" cellpadding="20">
      <tr>
		...
		if($_POST["endtime"]){
			$timeid = select_field("userprojectworktime where endtime=".$_GET["time"]." and userid=".$_GET["empid"],"id");
			if($timeid){
				$newendtime = mktime($_POST["endtime"][3],$_POST["endtime"][4],0,$_POST["endtime"][1],$_POST["endtime"][0],$_POST["endtime"][2]);
				$up = mysql_query("update userprojectworktime set endtime=".$newendtime." where endtime=".$_GET["time"]." and userid=".$_GET["empid"]);
				//echo "update userprojectworktime set endtime=".$newendtime." where endtime=".$_GET["time"]." and userid=".$_GET["empid"];
				//echo $timeid;
				if($up==1){
