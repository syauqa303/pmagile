        ...
        */
        ...
        <input type="submit" name="Submit" value="Show">
          ...
          <table width="100%" border="1" cellpadding="3" cellspacing="0" bordercolor="#FFFFFF">
          	...
          	<? 
          	$userq = mysql_query("select * from emp where type!=3 order by Name");
          	...
          	<tr>
          		...
          		for ($date=1;$date<=$mdays[$month];$date++){
          			...
          			$total = select_field("userworktime where starttime>$startTime and endtime<$endTime and userid=".$row["id"],"sum(endtime-starttime)");
					$userTotal += $total;
					$dayName = date("D",mktime(0,0,0,$month,$date,$year));
					if ($dayName=="Fri") {
						$style=" class=\"imp1\" ";
					...
					$over=$over>0?$over:0;
					$userOver += $over;
          			echo "<td $style >".round($total/3600)."/".round($over/3600) ."</td>";
      			}
